# Hi, I'm Kirthan Reddy 👋

Software Engineer building full-stack applications.

[LinkedIn](https://linkedin.com/in/kirthanreddy)

## 👨‍💻 About Me

I'm a Software Engineer interested in building reliable backend systems and full-stack applications.

I enjoy working across the stack, from designing APIs and working with databases to building user-facing applications and cloud-based services.

* 🎓 M.S. Computer Science — New York University
* 🎓 B.S. Biology — Penn State University
* 🤖 Exploring ML/AI topics such as recommender systems and classification

## 🛠️ Technologies

### Languages

<p>
  <img src="https://skillicons.dev/icons?i=java,python,typescript,javascript" />
</p>

### Frameworks & Libraries

<p>
  <img src="https://skillicons.dev/icons?i=react,nodejs,fastapi,graphql" />
</p>

### Databases

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb" />
</p>

### Testing

<p>
  <img src="https://skillicons.dev/icons?i=jest,vitest,cypress" />
</p>

### Cloud & Tools

<p>
  <img src="https://skillicons.dev/icons?i=aws,docker,jenkins,git" />
</p>

## 🚀 Featured Projects

### [OnlineTutors](https://github.com/kirth123/OnlineTutors)

Full-stack web application built with React, Node.js, MongoDB, and WebRTC.

### [StackOverflowAnalysis](https://github.com/kirth123/StackOverflowAnalysis)

Data analysis project exploring Stack Overflow data using Python and data-analysis tools.

### [CreditCardFraudDetection](https://github.com/kirth123/CreditCardFraudDetection)

Machine learning project focused on detecting fraudulent credit card transactions.

### [Codestroke](https://github.com/kirth123/Codestroke)

Software project combining application development with a healthcare-focused use case.

### [eBay Best Deals](https://github.com/kirth123/ebaybestdeals)

Application for finding and working with eBay deals.

## 🎓 Education

**New York University**
M.S. Computer Science · 2024

**Pennsylvania State University**
B.S. Biology · 2022

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=kirth123&show_icons=true&hide_border=true&rank_icon=github" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=kirth123&hide_border=true" />
</p>

---

<p align="center">
  <i>Thanks for stopping by!</i> 👋
</p>

